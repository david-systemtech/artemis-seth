...
A served conversation reaches the skills installed on the server, a remote pane's `/` menu lists them, a delegated task in a served conversation can be stopped, and the sidebar's groups can be put in the order you want.
...
**A delegated task in a served conversation can be stopped.** Every delegated-work row in a served pane drew a stop button, and pressing it failed with `Provider "artemis" cannot stop delegated tasks`: the server has answered the stop route since those rows first crossed the wire, but the served run never implemented the method the desktop checks before forwarding the press. It does now, posting the stop to the run on the server and showing the server's own sentence when it refuses — including for a task that outlived its turn, which is the one worth stopping. Desktop-only; no server rebuild needed.

**The sidebar's groups can be put in the order you want.** Groups were drawn in the order they were made and nothing could change it, so the group made last week for the work that matters most sat under every older one. A group heading can now be picked up and dropped between two others: the upper half of a group means before it, the lower half after it, and anywhere above or below the stack means its nearest end. One line shows where the group will land — under an open group's last row rather than under its heading, because between a heading and its own rows is not a place a group can go — and no line is drawn where the drop would change nothing. The heading's menu has "Move up" and "Move down" (`U` and `N`) for the same move without a drag. Desktop-only.
